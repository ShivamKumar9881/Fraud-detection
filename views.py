...
def predicts(request):
    if request.method == 'POST':
        ...
        l11 = request.POST.get('input11')

        loaded_model = joblib.load(os.path.join(BASE_DIR, 'media/rf_model.unknown'))
        result = loaded_model.predict([[l1, l2, l3, l4, l5, l6, l7, l8, l9, l10, l11]])
        print(result)
        if result == 0:
            print('not fraud')
        else:
            print('fraud')
        d = {'pred':result}
        return render(request, "user/results.html", d)
    else:
        return render(request, "user/userhome.html")
